...
import { Wallet, ShieldCheck, TrendingUp, Clock, RefreshCw } from "lucide-react";
...
const Dashboard = () => {
  ...
  const getAnalytics = async () => {
    try {
      ...
      setError(err.response?.data?.message || "Failed to sync pipeline analytics.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-8 max-w-7xl mx-auto font-sans text-slate-200 p-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 relative z-10">
          <div>
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
              Welcome back, <span className="bg-gradient-to-r from-[#34D399] to-emerald-400 bg-clip-text text-transparent">{user?.name || "Investor"}</span>
            </h1>
            <p className="text-xs sm:text-sm text-slate-400 mt-1 max-w-md">
              Secure client access portal. Monitoring your real-time asset nodes and historical yield metrics.
            </p>
          </div>
          <div className="shrink-0 self-start sm:self-center flex items-center gap-2 bg-[#090A0F]/40 px-3.5 py-2 border border-slate-800 rounded-xl shadow-inner">
            ...
            <span className="text-[10px] font-bold tracking-wider text-slate-300 uppercase">Node Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-gradient-to-b from-[#1F2937] to-[#18202F] border border-slate-800/80 p-5 rounded-2xl flex flex-col justify-between hover:border-slate-700/60 transition-all group shadow-md min-h-[128px]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wider block">Total Capital Invested</span>
          ...
          <div className="mt-4">
            {loading ? (
              <Skeleton.Button active size="small" className="w-32 block !bg-slate-800/50" />
            ) : (
              <span className="text-2xl font-bold text-white">
                ₦{summaryCards.totalCapitalInvested.toLocaleString("en-NG", { minimumFractionDigits: 2 })}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-b from-[#1F2937] to-[#18202F] border border-slate-800/80 p-5 rounded-2xl flex flex-col justify-between hover:border-slate-700/60 transition-all group shadow-md min-h-[128px]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wider block">My Active Pools</span>
            <div className="p-2 bg-[#090A0F]/40 rounded-xl group-hover:bg-[#090A0F]/60 transition-colors">
          ...
          <div className="mt-4">
            {loading ? (
              <Skeleton.Button active size="small" className="w-24 block !bg-slate-800/50" />
            ) : (
              <span className="text-2xl font-bold text-white">
                {summaryCards.activePoolsCount} <span className="text-xs font-medium text-slate-400">{summaryCards.activePoolsCount === 1 ? "Pool Allocation" : "Pool Allocations"}</span>
              </span>
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-b from-[#1F2937] to-[#18202F] border border-slate-800/80 p-5 rounded-2xl flex flex-col justify-between hover:border-slate-700/60 transition-all group shadow-md min-h-[128px]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wider block">Total Yield Accrued</span>
            <div className="p-2 bg-[#090A0F]/40 rounded-xl group-hover:bg-[#090A0F]/60 transition-colors">
          ...
          <div className="mt-4">
            {loading ? (
              <Skeleton.Button active size="small" className="w-28 block !bg-slate-800/50" />
            ) : (
              <span className="text-2xl font-bold text-emerald-400 tracking-tight block font-mono">
                ₦{summaryCards.totalYieldEarned.toLocaleString("en-NG", { minimumFractionDigits: 2 })}
              </span>
            )}
          </div>
        </div>
        ...
        <div className="bg-gradient-to-b from-[#1F2937] to-[#18202F] border border-slate-800/80 p-5 rounded-2xl flex flex-col justify-between hover:border-slate-700/60 transition-all group shadow-md min-h-[128px]">
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-[#9CA3AF] uppercase tracking-wider block">Next Payout Cycle</span>
            <div className="p-2 bg-[#090A0F]/40 rounded-xl group-hover:bg-[#090A0F]/60 transition-colors">
          ...
          <div className="mt-4">
            {loading ? (
              <Skeleton.Button active size="small" className="w-36 block !bg-slate-800/50" />
            ) : (
      ...
      <div className="bg-[#1F2937] border border-slate-800/60 rounded-2xl overflow-hidden shadow-lg shadow-[#090A0F]/20">
        <div className="p-6 border-b border-slate-800 bg-gradient-to-r from-[#1F2937] to-[#192231]">
          <h3 className="text-sm font-bold text-white tracking-wide">Your Capital Allocations</h3>
          <p className="text-[11px] text-slate-400 mt-0.5">Asset contract listings tied explicitly to your cryptographic user token signature.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="p-6 space-y-4 bg-[#1F2937]/40">
              <Skeleton active paragraph={{ rows: 3 }} title={false} className="custom-table-skeleton" />
            </div>
            ...
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="bg-[#090A0F]/30 text-slate-400 border-b border-slate-800 font-semibold tracking-wider text-[11px] uppercase">
                  ...
                  <th className="p-4">Target Return</th>
              ...
              <tbody className="divide-y divide-slate-800/40 text-slate-300 bg-[#1F2937]/40">
                {recentContracts.map((contract, index) => {
                  const nameOfPool = contract.poolName || contract.poolId?.name || contract.pool?.name || "Asset Pool Deployment";
                  const returnRate = contract.yieldRate || contract.poolId?.yieldRate || contract.pool?.yieldRate || "12% ARR";
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 font-mono text-slate-200">
                        ₦{(contract.principalAmount || contract.amount || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}
                      </td>
                      <td className="p-4 font-semibold text-emerald-400">
                        {returnRate}
                      </td>
                      <td className="p-4 font-mono text-slate-400">
                        ₦{(contract.yieldAccrued || contract.totalYield || 0).toLocaleString("en-NG", { minimumFractionDigits: 2 })}
                      </td>
